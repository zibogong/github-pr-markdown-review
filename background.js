// Handles GitHub API calls and token storage.
// Lives in the background worker so it can read chrome.storage synchronously
// before each request and hold the token out of the content script context.
...
async function handleApi({ method, path, body }) {
  ...
  if (!token) throw Object.assign(new Error('No GitHub token configured'), { code: 'no_token' });
  ...
  if (res.status === 401) {
    ...
    throw Object.assign(new Error('Token is invalid or expired — please re-enter it'), { code: 'invalid_token' });
  }
  if (!res.ok) {
